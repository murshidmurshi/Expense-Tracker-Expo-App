...
export default function SingleWork({ route, navigation }) {
    let { item, projectId, } = route.params;
    // console.log(totalAmount,admintotalAmount ,'totalAmount,admintotalAmount ');
    let { memberToken, MemberStyles, membertheme,memberdetail } = useContext(AuthContext)
    ...
    const [memberPractise, setMemberPRACtise] = useState([]);
    ...
    useFocusEffect(
        useCallback(() => {
            const SingleWork = async () => {
                ...
                await axios.get(`${Basepath}/work/view-work/${id}`)
                    .then(async (res) => {
                        console.log(res.data.WorkMemberPratise, 'WorkMemberPratiseWorkMemberPratiseWorkMemberPratise');
                        setMemberPRACtise(res.data.WorkMemberPratise)
                        ...
                        let otherMembers = (res?.data?.workmember || []).filter((item) => {
                            return item?.memberDetails?._id !== memberdetail._id;
                        });
                        let mainMember = (res?.data?.workmember || []).filter((item) => {
                            return item?.memberDetails?._id == memberdetail._id;
                        });
                        // console.log(mainMember[0]?.totalExpense);
                        setMainMember(mainMember)
                        // console.log(mainMember, 'mainMembermainMember');
    ...
    const SubmitExpense = async () => {
        ...
        await axios.post(`${Basepath}/expense/add-expense`, Newdata, { headers: { 'auth-token': memberToken } })
            .then((res) => {
    ...
    let SingletotalExpense = totalworkExpense || 0;
    let teammatesExpense = totalworkExpense - (mainMember[0]?.totalExpense) || 0;
    ...
    const styles = StyleSheet.create({
        ...
        ExpenseDescDiv: {
            paddingHorizontal: 10,
            backgroundColor: MemberStyles.DescriptionBackGround,
            borderRadius: 7,
            paddingVertical: 4
        ...
        DetailExpenseDivHeader: {
            backgroundColor: membertheme == 'dark' ? 'grey' : 'rgba(212, 209, 209, 0.132)',
            justifyContent: 'space-between',
            flexDirection: 'row',
            borderRadius: 8,
            padding: 12,
            ...
            <ScrollView >
                <TouchableOpacity activeOpacity={5} onPress={CloseBottomSheet}>
                    ...
                    <View style={[styles.MainHeader,]}>
                        ...
                        <Text numberOfLines={1} style={[styles.headerText,]}>{item?.name}</Text>
                    ...
                    <View style={styles.SingleWorkMainDiv}>
                                ...
                                <View style={styles.SingleMember}>
                                    ...
                                    <View style={styles.MemberLocationandMessage}>
                                        <View style={styles.MemberLocationDiv}>
                                            <Ionicons name='location-outline' color={MemberStyles.GreenColor} size={14} />
                                            {workAdmin.address ? (
                                        ...
                                        <Ionicons name='chatbubble-ellipses-outline' color={MemberStyles.PurpleColor} size={22} />
                                    </View>
                                </View>
                                ...
                                <TouchableOpacity activeOpacity={2} style={styles.chartDetailDiv}>
                                    <View style={styles.RecievedDiv}>
                                        ...
                                        <View style={styles.AmountDiv}>
                                            ...
                                            <Text style={[styles.ChartAmountText,]}>{SingletotalExpense?.toLocaleString('en-IN')}</Text>
                                        </View>
                                    </View>
                                    <View style={styles.RecievedDiv}>
                                        <Text style={styles.DetailHeaderText}>Teammates expense</Text>
                                        <View style={styles.AmountDiv}>
                                            <Text style={[styles.paymentRupeesText,]}> - ₹</Text>
                                            <Text style={[styles.ChartAmountText,]}>{teammatesExpense?.toLocaleString('en-IN')}</Text>
                                        </View>
                                    </View>

                                    <Text style={{ height: 0.5, backgroundColor: 'grey', marginVertical: 8 }}></Text>
                                    <View style={styles.RecievedDiv}>
                                        <Text style={[styles.DetailHeaderText, { fontWeight: '500' }]}>Your expense</Text>
                                        <View style={styles.AmountDiv}>
                                            <Text style={[styles.paymentRupeesText,]}> ₹</Text>
                                            <Text style={[styles.ChartAmountText, { fontWeight: '500', fontSize: 18, }]}>
                                                {/* {(SingletotalExpense) > teammatesExpense ? "0"?.toLocaleString('en-IN') : (SingletotalExpense - teammatesExpense)?.toLocaleString('en-IN')} */}
                                                {mainMember[0]?.totalExpense || 0}
                                            </Text>
                                        </View>
                                    </View>
                                </TouchableOpacity>
                        ...
                        <View>
                            <Text style={styles.InvolveWordMemberDivHeader}>My Expenses </Text>
                            {mainMember.length == 0 && (
                            ...
                            {mainMember.map((item, index) => {
                                return (
                                    <>
                                        <TouchableOpacity key={index} style={[styles.MemberandExpenseDiv, { marginVertical: 10 }]} onPress={() => handleBotomSheet(item, index)} activeOpacity={0.8}>
                        ...
                        <View style={styles.InvolveWordMemberDiv}>
                            <Text style={styles.InvolveWordMemberDivHeader}>Teammates </Text>
                            <View style={styles.InvolveMemberDetailDiv}>
                                ...
                                {workmember?.map((item, index) => {
                                    // console.log(item.memberDetails._id, 'ITEMMM999999999999');
                                    return (
                                        <>
                                            <TouchableOpacity key={index} onPress={() => handleBotomSheet(item, index)} activeOpacity={0.8} style={styles.SingleInvolveMemberDiv}>
                                                <Text style={styles.RoleText}>
                                                    ...
                                                    {/* {console.log(item.memberDetails._id===memberPractise[index]._id,'memberPractisememberPractisev')} */}
                                                    {/* {console.log(memberPractise[0]._id)} */}
                                                   {/* {workmember[index].memberDetails._id===memberPractise[index]._id?memberPractise[index].roles:''} */}
                            ...
                            <View style={styles.singleMemberDiv}>
                                ...
                                <View style={styles.SingleMember}>
                                    ...
                                    <View style={styles.MemberDetailDiv}>
                                        ...
                                        <View style={styles.PhoneIconandNumber}>
                                            <Ionicons style={styles.PhoneIcon} name='call-outline' color={MemberStyles.GreenColor} size={15} />
                                        ...
                                        <View key={index} style={styles.DetailExpenseDiv}>
                                            ...
                                            <View style={styles.WorkExpenseDiv}>
                                                ...
                                                <TouchableOpacity activeOpacity={0.8} onPress={() => handleToggleDesc(index)} >
                                                    <Ionicons name={showdesc === index ? 'chevron-up-outline' : 'chevron-down-outline'} size={17} color={MemberStyles.GreyWhite} />
                                                </TouchableOpacity>
                    ...
                    <View style={styles.AddExpenseDiv}>
                        <View style={styles.modalViewInside}>
                            ...
                            <View>
                                ...
                                <TextInput placeholderTextColor={MemberStyles.GreyWhite} keyboardType='numeric' onChangeText={(value) => ModelInputChange('amount', value)} style={styles.modelInput} placeholder='Enter expense amount' />
                            ...
                            <View>
                                ...
                                <TextInput placeholderTextColor={MemberStyles.GreyWhite} numberOfLines={3} multiline onChangeText={(value) => ModelInputChange('description', value)} style={[styles.modelInput, { height: 55 }]} placeholder='Enter expenses description' />
